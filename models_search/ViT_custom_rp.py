...
class SwinTransformerSys(nn.Module):
    ...
    def __init__(self,  img_size=480, patch_size=4, in_chans=3, num_classes=2,
                 embed_dim=192, depths=[2, 2, 2, 2], depths_decoder=[1, 2, 2, 2, 2, 2], num_heads=[3, 6, 12, 24],
        ...
        self.num_heads_decoder = [3, 3, 3, 6, 12, 24]
        ...
        for i_layer in range(self.num_layers_decoder):
            concat_linear = nn.Linear(2 * int(embed_dim * 2 ** (self.num_layers_decoder - 1 - i_layer)),
                                      int(embed_dim * 2 ** (
                                                  self.num_layers_decoder - 1 - i_layer))) if i_layer > 0 else nn.Identity()
            if i_layer == 0:  # init 涉及resolution的部分用num_layers，其他用num_layers_decoder
                ...
                layer_up = BasicLayer_up(dim=int(embed_dim * 2 ** (self.num_layers - 1 - i_layer)),
                                         input_resolution=(
                                         int(patches_resolution[0] // (2 ** (self.num_layers - 1 - i_layer))),
                                         int(patches_resolution[1] // (2 ** (self.num_layers - 1 - i_layer)))),
                                         depth=depths_decoder[(self.num_layers_decoder - 1 - i_layer)],
                                         num_heads=self.num_heads_decoder[(self.num_layers_decoder - 1 - i_layer)],
                                         window_size=window_size,
                                         mlp_ratio=self.mlp_ratio,
                                         qkv_bias=qkv_bias, qk_scale=qk_scale,
                                         drop=drop_rate, attn_drop=attn_drop_rate,
                                         drop_path=dpr_up[
                                                   sum(depths_decoder[:(self.num_layers_decoder - 1 - i_layer)]):sum(
                                                       depths_decoder[:(self.num_layers_decoder - 1 - i_layer) + 1])],
                                         norm_layer=norm_layer,
                                         upsample=PatchExpand if (i_layer < self.num_layers_decoder - 1) else None,
                                         use_checkpoint=use_checkpoint)
        ...
        if self.final_upsample == "expand_first":
            ...
            self.up = FinalPatchExpand_X4(input_resolution=(160 // patch_size, 480 // patch_size), dim_scale=4,
                                          dim=embed_dim)
            self.output = nn.Conv2d(in_channels=self.embed_dim // 4, out_channels=self.num_classes, kernel_size=1,
                                    bias=False)
...
class Discriminator(nn.Module):
    ...
    """
    def __init__(self, args, img_size=32, patch_size=None, in_chans=3, num_classes=2, embed_dim=None, depth=7,
                 num_heads=4, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop_rate=0., attn_drop_rate=0.,
                 drop_path_rate=0., hybrid_backbone=None, norm_layer=nn.LayerNorm):
        super().__init__()
        self.num_classes = num_classes
